...
class UnicodeProcessor {
  ...
  static Future<UnicodeProcessor> load(String path) async {
    final jsonStr = path.startsWith('assets/')
...
class TextToSpeech {
  ...
  Future<Map<String, dynamic>> _infer(
    ...
    var noisyLatentRaw = latentData['noisyLatent'];
...
Future<TextToSpeech> loadTextToSpeech(String onnxDir) async {
  debugPrint('Loading TTS models from $onnxDir');
...
Future<Style> _loadVoiceStyleFromFile(String voice) async {
  final path = 'assets/voice_styles/$voice.json';
  ...
  return Style(
    await OrtValue.fromList(Float32List.fromList(ttlData), ttlShape),
    await OrtValue.fromList(Float32List.fromList(dpData), dpShape),
    ttlShape,
    dpShape,
  );
}
...
Future<Map<String, OrtSession>> _loadOnnxAll(String dir) async {
  ...
  final sessions = await Future.wait(models.map((name) async {
    ...
    debugPrint('Loading $name.onnx from $path');
...
class SupertonicService {
  static final SupertonicService _instance = SupertonicService._internal();
  static SupertonicService get instance => _instance;
  SupertonicService._internal();
  ...
  bool get isLoaded => _isLoaded && _tts != null;
  bool get isLoading => _isLoading;
  String? get currentVoice => _currentVoice;
  int get sampleRate => _tts?.sampleRate ?? 44100;

  Future<bool> canLoad() async {
    return await SupertonicModelService.instance.hasAllModels();
  }

  Future<void> loadModels() async {
    if (_isLoaded) {
      debugPrint('Supertonic models already loaded');
      return;
    }

    if (_isLoading) {
      debugPrint('Supertonic models are being loaded');
      return;
    }
    ...
    try {
      ...
      debugPrint('Error loading Supertonic models: $e');
      debugPrint('StackTrace: $stackTrace');
      _isLoaded = false;
      _tts = null;
      throw Exception('Failed to load Supertonic models: $e');
    } finally {
      _isLoading = false;
    }
  }

  Future<void> loadVoiceStyle(String voice) async {
    if (_currentVoice == voice && _currentStyle != null) {
      debugPrint('Voice style $voice already loaded');
      return;
    }

    if (!availableSupertonicVoices.contains(voice)) {
      throw ArgumentError('Invalid voice: $voice');
    }

    debugPrint('Loading voice style: $voice');
    _currentStyle = await _loadVoiceStyleFromFile(voice);
    _currentVoice = voice;
    debugPrint('Voice style $voice loaded');
  }
  ...
  Future<String?> synthesize(
    ...
    if (_tts == null) {
      throw Exception('Supertonic models not loaded');
    }

    await loadVoiceStyle(voice);
    ...
    try {
      ...
      final duration = result['duration'] is List<double>
          ? result['duration']
      ...
      final outputPath = '${tempDir.path}/supertonic_$timestamp.wav';
      ...
      debugPrint('Audio saved to: $outputPath');
      return outputPath;
    } catch (e, stackTrace) {
  ...
  void dispose() {
    ...
    _isLoaded = false;
    _isLoading = false;
    debugPrint('SupertonicService disposed');
  }
}
